...
use Storage;
use Auth;
class PengendalianpelaksanaController extends Controller
{
    ...
    {
        return view('pengendalian-pelaksana');
    }
    ...
    public function insert(Request $request)
    {
        // dd($request);
        $user = Auth::user()->id;
        $data =new Pengendalian_pelaksana([
            'user_id'        => $user,
            ...
            if($request->file('file1') == null ){
                ...
                $pathnam->move(public_path('storage/pengendalian_pelaksana'),$new_name);
                $data->file1 = $new_name;
            }
            ...
            if($request->file('file2') == null ){
                ...
                $new_name = date('y-m-d').'-'.$name;
                $pathsat->move(public_path('storage/pengendalian_pelaksana'),$new_name);
                $data->file2 = $new_name;
            }
            if($request->file('file3') == null ){
                ...
                $new_name = date('y-m-d').'-'.$name;
                $pathdua->move(public_path('storage/pengendalian_pelaksana'),$new_name);
                $data->file3 = $new_name;
            }
            if($request->file('file4') == null ){
                ...
                $new_name = date('y-m-d').'-'.$name;
                $path->move(public_path('storage/pengendalian_pelaksana'),$new_name);
                $data->file4 = $new_name;
            }
        $data->save();
