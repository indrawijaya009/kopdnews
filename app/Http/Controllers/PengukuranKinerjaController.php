...
class PengukuranKinerjaController extends Controller
{
    ...
    {
        return view('pengukuran-kinerja');
    }
    ...
    public function insert(Request $request)
    {
        ...
        $data =new Pengukuran_kinerja([
            ...
            if($request->file('file1') == null ){
                ...
                $pathnam->move(public_path('storage/pengukuran_kinerja'),$new_name);
                $data->file1 = $new_name;
            }
            ...
            if($request->file('file2') == null ){
                ...
                $new_name = date('y-m-d').'-'.$name;
                $pathsat->move(public_path('storage/pengukuran_kinerja'),$new_name);
                $data->file2 = $new_name;
            }
            if($request->file('file3') == null ){
                ...
                $new_name = date('y-m-d').'-'.$name;
                $pathdua->move(public_path('storage/pengukuran_kinerja'),$new_name);
                $data->file3 = $new_name;
            }
            if($request->file('file4') == null ){
                ...
                $new_name = date('y-m-d').'-'.$name;
                $path->move(public_path('storage/pengukuran_kinerja'),$new_name);
                $data->file4 = $new_name;
            }
        $data->save();
